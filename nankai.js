...
async function init(line) {
    ...
    */
    ...
    var types = {
        普通: '보통',    //난카이선 계통 완행은 보통
        各停: '각역정차',  //고야선은 계통 완행은 각정으로 씀. 본선은 정차하지 못하는 역이 2곳 있음
        区急: '구간급행',
        準急: '준급',
        空港急行: '공항급행',
        サザン: '서던',
    ...
    result.forEach((e, i) => {
        data.forEach((e) => {
            ...
            var type = names[e.type] ? names[e.type] : 'Type: ' + e.type
            result[index][e.updn].push({
                trainNo: e.trainNo,
                sts: e.sts,
                terminal: e.terminal,
                delay: e.delay,
                type: types[type] ? types[type] : 'Type: ' + type
            });
        });
    });
